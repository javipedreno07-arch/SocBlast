    ...
    <div style={{display:'flex',gap:'3px'}}>
      {weeks.map((wk,wi)=>(
        <div key={wi} style={{display:'flex',flexDirection:'column',gap:'3px'}}>
          {wk.map((d,di)=>(
            <div key={di} title={`${d.date.toLocaleDateString('es-ES')} · ${d.count} sesiones`}
    ...
    <div style={{minHeight:'100vh',background:BG,fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif",color:'#0f172a'}}>
      ...
      <div style={{maxWidth:1080,margin:'0 auto',padding:'24px 24px 64px',display:'grid',gridTemplateColumns:'300px 1fr',gap:18,alignItems:'start'}}>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:16}}>
          ...
          {/* CTAs */}
          <div className="s1" style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
            <button className="pb" onClick={()=>navigate('/sesion')} style={{padding:'15px',borderRadius:14,background:'linear-gradient(135deg,#4f46e5,#6366f1)',border:'none',color:'#fff',fontSize:15,fontWeight:800,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:9,boxShadow:'0 6px 24px rgba(79,70,229,0.3)',transition:'all .2s'}}>
              <Icon name="bolt" size={16} color="#fff"/> Jugar sesión
            </button>
            <button className="pb" onClick={()=>navigate('/lab')} style={{padding:'15px',borderRadius:14,background:'linear-gradient(135deg,#047857,#059669)',border:'none',color:'#fff',fontSize:15,fontWeight:800,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:9,boxShadow:'0 6px 24px rgba(5,150,105,0.3)',transition:'all .2s'}}>
          ...
          <div className="s1 sb-card" style={{overflow:'hidden'}}>
            <div style={{display:'flex',borderBottom:'1px solid #e8eaf0'}}>
              {[{i:0,l:'Sesiones competitivas',c:ACC},{i:1,l:'Laboratorio SOC',c:'#059669'}].map(t=>(
                <button key={t.i} onClick={()=>setCarruselIdx(t.i)}
                  style={{flex:1,padding:'13px',background:'none',border:'none',cursor:'pointer',fontSize:13,fontWeight:600,color:carruselIdx===t.i?t.c:'#94a3b8',borderBottom:carruselIdx===t.i?`2.5px solid ${t.c}`:'2.5px solid transparent',transition:'all .2s'}}>
                  {t.l}
                </button>
              ))}
            </div>
            {carruselIdx===0 ? (
              <div style={{padding:'24px 28px',background:'linear-gradient(135deg,#1e1b4b 0%,#312e81 50%,#3730a3 100%)',display:'grid',gridTemplateColumns:'1fr auto',gap:24,alignItems:'center'}}>
                <div>
                  <div style={{display:'inline-flex',alignItems:'center',gap:6,padding:'3px 11px',borderRadius:100,background:'rgba(255,255,255,0.1)',marginBottom:12}}>
                    ...
                    <span style={{fontSize:10,color:'#a5b4fc',fontWeight:700,letterSpacing:'2px'}}>COMPETITIVO</span>
                  </div>
                  <h3 style={{fontSize:18,fontWeight:900,color:'#fff',marginBottom:8,lineHeight:1.2}}>Demuestra tu nivel.<br/><span style={{color:'#a5b4fc'}}>Gana puntos.</span></h3>
                  <p style={{fontSize:12,color:'rgba(255,255,255,0.5)',lineHeight:1.7,marginBottom:14}}>La IA genera escenarios únicos. Tu OVR sube con cada buena sesión.</p>
                  <div style={{display:'flex',gap:6,flexWrap:'wrap',marginBottom:16}}>
                    {['Timer activo','Puntos y arenas','Skills evaluadas','Ranking'].map((f,i)=>(
                      <span key={i} style={{fontSize:10,padding:'3px 9px',borderRadius:6,background:'rgba(255,255,255,0.1)',color:'rgba(255,255,255,0.75)',fontWeight:500}}>{f}</span>
                    ))}
                  </div>
                  <button className="pb" onClick={()=>navigate('/sesion')} style={{padding:'10px 24px',borderRadius:100,background:'#fff',border:'none',color:ACC,fontSize:13,fontWeight:800,cursor:'pointer',boxShadow:'0 4px 16px rgba(0,0,0,0.15)',transition:'all .2s'}}>
                    Jugar ahora →
                  </button>
                </div>
              ...
              <div style={{padding:'24px 28px',background:'linear-gradient(135deg,#064e3b 0%,#065f46 50%,#047857 100%)',display:'grid',gridTemplateColumns:'1fr auto',gap:24,alignItems:'center'}}>
                <div>
                  <div style={{display:'inline-flex',alignItems:'center',gap:6,padding:'3px 11px',borderRadius:100,background:'rgba(255,255,255,0.1)',marginBottom:12}}>
                  ...
                  <h3 style={{fontSize:18,fontWeight:900,color:'#fff',marginBottom:8,lineHeight:1.2}}>Investiga sin límite.<br/><span style={{color:'#6ee7b7'}}>Profundidad real.</span></h3>
                  <p style={{fontSize:12,color:'rgba(255,255,255,0.5)',lineHeight:1.7,marginBottom:14}}>SIEM, Log Explorer, Network Map y evaluación IA.</p>
                  <div style={{display:'flex',gap:6,flexWrap:'wrap',marginBottom:16}}>
                    {['SIEM queries','Log Explorer','Network Map','IA evalúa'].map((f,i)=>(
                      <span key={i} style={{fontSize:10,padding:'3px 9px',borderRadius:6,background:'rgba(255,255,255,0.1)',color:'rgba(255,255,255,0.75)',fontWeight:500}}>{f}</span>
                    ))}
                  </div>
                  <button className="pb" onClick={()=>navigate('/lab')} style={{padding:'10px 24px',borderRadius:100,background:'#fff',border:'none',color:'#047857',fontSize:13,fontWeight:800,cursor:'pointer',boxShadow:'0 4px 16px rgba(0,0,0,0.15)',transition:'all .2s'}}>
                    Entrar al Lab →
                  </button>
                </div>
                <svg width="110" height="110" viewBox="0 0 170 170" fill="none">
                  ...
                  <text x="20" y="78" fontFamily="monospace" fontSize="8" fill="#60a5fa">$ _</text>
                </svg>
              </div>
            )}
          </div>
          ...
          <div className="s2" style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14}}>
            ...
            <div className="sb-card-sm" style={{padding:'18px'}}>
              ...
              <div style={{marginTop:12,padding:'11px 13px',borderRadius:10,background:'#fff5f5',border:'1px solid #fecaca'}}>
                ...
                <button onClick={()=>navigate('/sesion')} style={{width:'100%',padding:'7px',borderRadius:8,background:'#ef4444',border:'none',color:'#fff',fontSize:11,fontWeight:700,cursor:'pointer'}}>Entrenar →</button>
              </div>
            </div>
          </div>
          ...
          <div className="s2 sb-card-sm" style={{padding:'18px 20px'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14}}>
              ...
              <button onClick={()=>navigate('/sesion')} style={{fontSize:11,color:ACC,background:'none',border:'none',cursor:'pointer',fontWeight:700}}>Nueva →</button>
            </div>
            {historial.length===0 ? (
              <div style={{textAlign:'center',padding:'16px 0'}}>
                <p style={{fontSize:12,color:'#94a3b8',marginBottom:10}}>Sin sesiones aún. ¡Empieza a construir tu perfil!</p>
                <button className="sb-btn-primary pb" onClick={()=>navigate('/sesion')} style={{padding:'9px 22px',fontSize:12}}>Empezar ahora</button>
              </div>
            ) : historial.slice(0,5).map((s,i)=>{
              const copasGan=s.resultado?.copas_ganadas||0, media=s.resultado?.media_puntuacion||0, pos=copasGan>=0;
              return (
                <div key={i} className="hr" style={{display:'flex',alignItems:'center',gap:10,padding:'9px 10px',borderRadius:10,background:'#f8f7ff',border:'1px solid #ede9fe',marginBottom:7,transition:'all .15s'}}>
                  <div style={{width:28,height:28,borderRadius:8,background:pos?'#ecfdf5':'#fef2f2',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
                    <Icon name="trend" size={13} color={pos?'#059669':'#ef4444'}/>
                  </div>
                  <div style={{flex:1,minWidth:0}}>
                    <p style={{fontSize:12,color:'#0f172a',fontWeight:600,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{s.titulo||'Sesión SOC'}</p>
                    <p style={{fontSize:10,color:'#94a3b8'}}>{media}/20 pts</p>
                  </div>
                  <span style={{fontSize:12,fontWeight:800,color:pos?'#059669':'#ef4444'}}>{pos?'+':''}{copasGan}</span>
                </div>
              );
            })}
          </div>
