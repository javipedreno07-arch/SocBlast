...
export default function RankingPage() {
  ...
  const TIERS_ARR = ['Bronce','Plata','Oro','Diamante'];
  const filtros = ['Todos',...TIERS_ARR];
    ...
    <>
      ...
      <div style={{minHeight:'100vh',background:'linear-gradient(150deg,#f0f4ff 0%,#f8f9ff 40%,#f5f0ff 100%)',fontFamily:"'Inter',-apple-system,sans-serif",color:'#0f172a'}}>
        ...
        <div style={{maxWidth:'860px',margin:'0 auto',padding:'32px 40px 60px'}}>
          ...
          {/* Mi posición */}
          {miPosicion && (
            <div className="fade-up" style={{padding:'20px 24px',borderRadius:'14px',backgroundColor:'#fff',border:`2px solid ${ACC}20`,marginBottom:'20px',boxShadow:`0 4px 20px ${ACC}10`,position:'relative',overflow:'hidden'}}>
          ...
          {/* Header tabla */}
          <div style={{display:'grid',gridTemplateColumns:'56px 1fr 140px 120px',padding:'8px 16px',marginBottom:'6px'}}>
            {['POS','ANALISTA','ARENA','COPAS'].map((h,i)=>(
              <span key={i} style={{fontSize:'10px',color:'#94a3b8',fontWeight:700,letterSpacing:'1.5px',textAlign:i>1?'right':'left'}}>{h}</span>
            ))}
          </div>
              ...
              {filtrados.map((j,i)=>{
                ...
                return (
                  <div key={i} className="row"
                    style={{display:'grid',gridTemplateColumns:'56px 1fr 140px 120px',alignItems:'center',padding:'14px 16px',borderRadius:'12px',backgroundColor:esMio?`${ACC}06`:'#fff',border:esMio?`1.5px solid ${ACC}20`:'1px solid #e8eaf0',boxShadow:'0 1px 4px rgba(0,0,0,0.04)',position:'relative',overflow:'hidden'}}>
