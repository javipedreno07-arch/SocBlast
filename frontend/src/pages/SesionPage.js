...
const LogoSpinner = ({ text, subtext, countdown }) => (
  <div style={{ minHeight:'100vh', backgroundColor:BG, display:'flex', alignItems:'center', justifyContent:'center', fontFamily:"'Inter',sans-serif" }}>
    <div style={{ textAlign:'center' }}>
      ...
      <p style={{ color:ACC, fontSize:'15px', fontWeight:700, letterSpacing:'2px', marginBottom:'8px' }}>{text}</p>
      <p style={{ color:T4, fontSize:'13px', letterSpacing:'1px', marginBottom: countdown ? '16px' : 0 }}>{subtext}</p>
      {countdown !== undefined && countdown > 0 && (
        <div style={{ display:'inline-flex', alignItems:'center', gap:'8px', padding:'6px 16px', borderRadius:'20px', backgroundColor:'#f1f5f9', border:'1px solid #e2e8f0' }}>
          <div style={{ width:'6px', height:'6px', borderRadius:'50%', backgroundColor:'#22c55e', animation:'pulse 1s infinite' }}/>
          <span style={{ fontSize:'12px', color:T3, fontFamily:'monospace' }}>~{countdown}s restantes</span>
        </div>
      )}
    </div>
  </div>
  ...
  return (
    <div style={{ minHeight:'100vh', backgroundColor:BG, display:'flex', alignItems:'center', justifyContent:'center', fontFamily:"'Inter',sans-serif" }}>
      <div style={{ textAlign:'center' }}>
        ...
        <p style={{ color:T4, fontSize:'13px', fontFamily:'monospace', marginTop:'20px', letterSpacing:'3px' }}>
...
const Timer = ({ segundos, limite }) => {
  ...
  return (
    <div style={{ position:'relative', display:'flex', alignItems:'center', gap:'8px', padding:'6px 14px', borderRadius:'10px', backgroundColor:critico?'rgba(239,68,68,0.08)':'#f8fafc', border:`1px solid ${critico?'rgba(239,68,68,0.4)':BD}`, animation:critico?'timerPulse .8s infinite':'none' }}>
...
const ModalSalir = ({ onConfirm, onCancel }) => (
  <div style={{ position:'fixed', inset:0, backgroundColor:'rgba(0,0,0,0.5)', zIndex:9999, display:'flex', alignItems:'center', justifyContent:'center', fontFamily:"'Inter',sans-serif" }}>
    <div style={{ maxWidth:'400px', width:'90%', padding:'36px', borderRadius:'18px', backgroundColor:CARD, border:'1px solid #e2e8f0', boxShadow:'0 20px 60px rgba(0,0,0,0.2)', textAlign:'center' }}>
      <div style={{ fontSize:'44px', marginBottom:'16px' }}>⚠️</div>
      <h2 style={{ fontSize:'20px', fontWeight:800, color:T1, marginBottom:'10px' }}>¿Abandonar la sesión?</h2>
      ...
      <div style={{ display:'flex', gap:'12px' }}>
        <button onClick={onCancel} style={{ flex:1, padding:'13px', borderRadius:'10px', backgroundColor:'#f8fafc', border:'1px solid #e2e8f0', color:T2, fontSize:'14px', fontWeight:600, cursor:'pointer' }}>Continuar sesión</button>
        <button onClick={onConfirm} style={{ flex:1, padding:'13px', borderRadius:'10px', backgroundColor:'rgba(239,68,68,0.08)', border:'1px solid rgba(239,68,68,0.3)', color:'#ef4444', fontSize:'14px', fontWeight:700, cursor:'pointer' }}>Sí, salir (-10 🏆)</button>
      </div>
    </div>
  </div>
);
...
export default function SesionPage() {
  ...
  const generar = async () => {
    ...
    try {
      ...
      pararCountdown();
      if (cancelRef.current) return; // usuario canceló
      setSesion(r.data);
      setTiempoRestante(TIEMPOS[tierArena]||1200);
      setFase('briefing');
    } catch {
      pararCountdown();
      if (!cancelRef.current) alert('Error generando sesión');
      setFase('intro');
    }
  };
  ...
  const intentarSalir = () => {
    // Solo mostrar modal si hay sesión activa
    if (fase === 'activa' || fase === 'feedback') {
  ...
  const confirmarSalir = async () => {
    ...
    try {
      await axios.put(`${API}/api/me/copas`, null, {
        params: { copas_delta: -10 },
  ...
  const siguiente = () => {
    if (incIdx+1 < sesion.incidentes.length) {
      setIncIdx(i=>i+1); setEvaluacion(null); resetEtapas(); setSkillsMejoradas({});
  ...
  const css = `
    ...
    @keyframes timerPulse{0%,100%{box-shadow:0 0 0 0 rgba(239,68,68,.35);}50%{box-shadow:0 0 0 8px rgba(239,68,68,0);}}
    @keyframes glow{0%,100%{box-shadow:0 0 20px ${ACC}30;}50%{box-shadow:0 0 48px ${ACC}55;}}
    ...
    .accion-btn:hover{border-color:${ACC}60!important;background:${ACC}08!important;}
    .diag-opt:hover{border-color:${ACC}50!important;}
    .log-card:hover{border-color:${ACC}40!important;}
  ...
  const Navbar = ({ center, onBack, hideBack }) => (
    <nav style={{ height:'56px', display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 36px', backgroundColor:'rgba(255,255,255,0.95)', backdropFilter:'blur(20px)', borderBottom:'1px solid #e8eaf0', boxShadow:'0 1px 8px rgba(0,0,0,0.06)', fontFamily:"'Inter',sans-serif", flexShrink:0 }}>
      ...
      {!hideBack && (
        <button onClick={intentarSalir} style={{ background:'none', border:'1px solid #e2e8f0', color:T3, padding:'6px 14px', borderRadius:'8px', fontSize:'12px', cursor:'pointer', fontWeight:500 }}>
      ...
      <div style={{ minHeight:'100vh', backgroundColor:BG, fontFamily:"'Inter',sans-serif" }}>
        ...
        <div style={{ maxWidth:'680px', margin:'0 auto', padding:'44px 28px' }}>
          <div className="fade-up">
            ...
            <div style={{ padding:'22px 24px', borderRadius:'16px', backgroundColor:CARD, border:`1px solid ${BD}`, marginBottom:'24px', boxShadow:'0 2px 8px rgba(0,0,0,0.05)' }}>
  ...
  if (fase==='cargando') return (
    <>
      <style>{css}</style>
      <div style={{ minHeight:'100vh', backgroundColor:BG, display:'flex', flexDirection:'column', fontFamily:"'Inter',sans-serif" }}>
        <nav style={{ height:'56px', display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 36px', backgroundColor:'rgba(255,255,255,0.95)', borderBottom:'1px solid #e8eaf0' }}>
          <img src="/logosoc.png" style={{ height:'28px' }}/>
          <button onClick={cancelarCarga} style={{ background:'none', border:'1px solid #e2e8f0', color:T3, padding:'6px 14px', borderRadius:'8px', fontSize:'12px', cursor:'pointer' }}>✕ Cancelar</button>
        </nav>
        <div style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center' }}>
          <div style={{ textAlign:'center' }}>
            <img src="/logosoc.png" alt="Cargando" style={{ width:'64px', height:'64px', animation:'spinLogo 1s linear infinite', marginBottom:'24px' }}/>
            <p style={{ color:ACC, fontSize:'15px', fontWeight:700, letterSpacing:'2px', marginBottom:'8px', fontFamily:'monospace' }}>GENERANDO ESCENARIO...</p>
            <p style={{ color:T4, fontSize:'13px', marginBottom:'20px' }}>La IA construye amenazas para {arena}</p>
            <div style={{ display:'inline-flex', alignItems:'center', gap:'8px', padding:'8px 18px', borderRadius:'20px', backgroundColor:'#f1f5f9', border:'1px solid #e2e8f0' }}>
              <div style={{ width:'6px', height:'6px', borderRadius:'50%', backgroundColor:'#22c55e', animation:'pulse 1s infinite' }}/>
  ...
  if (fase==='briefing') return (
    <>
      <style>{css}</style>
      <div style={{ minHeight:'100vh', backgroundColor:BG, fontFamily:"'Inter',sans-serif", display:'flex', flexDirection:'column' }}>
        <Navbar right={<button onClick={()=>setFase('intro')} style={{ background:'none', border:'1px solid #e2e8f0', color:T3, padding:'6px 14px', borderRadius:'8px', fontSize:'12px', cursor:'pointer' }}>✕ Cancelar</button>} hideBack/>
        <div style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center', padding:'32px 24px' }}>
          <div className="fade-up" style={{ maxWidth:'600px', width:'100%' }}>
            <div style={{ padding:'40px', borderRadius:'22px', backgroundColor:CARD, border:`1px solid ${BD}`, boxShadow:'0 12px 48px rgba(0,0,0,0.08)', position:'relative', overflow:'hidden' }}>
            ...
            <div className="fade-up">
              <div style={{ padding:'36px', borderRadius:'22px', backgroundColor:CARD, border:`1.5px solid ${sc}20`, boxShadow:'0 8px 40px rgba(0,0,0,0.07)', marginBottom:'16px', position:'relative', overflow:'hidden' }}>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'12px', marginBottom:'22px' }}>
                  {[{label:'Calidad análisis',value:`${evaluacion.puntuacion_calidad}/12`,color:'#22c55e'},{label:'Velocidad',value:`${evaluacion.puntuacion_velocidad}/5`,color:'#f59e0b'},{label:'Sin pistas',value:`${evaluacion.puntuacion_pistas}/3`,color:ACC}].map((s,i)=>(
                    <div key={i} style={{ padding:'16px', borderRadius:'12px', backgroundColor:BG, border:`1px solid ${BD}`, textAlign:'center' }}>
  ...
  if (fase==='finalizada' && resultado) return (
    <>
      <style>{css}</style>
      <div style={{ minHeight:'100vh', backgroundColor:BG, display:'flex', alignItems:'center', justifyContent:'center', padding:'28px', fontFamily:"'Inter',sans-serif" }}>
  ...
  return (
    <>
      <style>{css}</style>
      {mostrarModalSalir && <ModalSalir onConfirm={confirmarSalir} onCancel={()=>setMostrarModalSalir(false)}/>}
      <div style={{ minHeight:'100vh', backgroundColor:BG, fontFamily:"'Inter',sans-serif", color:T1, display:'flex', flexDirection:'column' }}>

        {/* NAVBAR */}
        <div style={{ height:'52px', display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 28px', backgroundColor:'rgba(255,255,255,0.97)', backdropFilter:'blur(20px)', borderBottom:`1px solid ${BD}`, flexShrink:0, boxShadow:'0 1px 8px rgba(0,0,0,0.06)' }}>
              ...
              {ETAPAS.map((e,i)=>{
                ...
                return (
                  <div key={i} style={{ display:'flex', alignItems:'center', gap:'4px' }}>
                    <div style={{ padding:'3px 8px', borderRadius:'5px', fontSize:'9px', fontFamily:'monospace', fontWeight:700, letterSpacing:'1px', backgroundColor:done?'rgba(34,197,94,0.1)':active?`${ACC}10`:'transparent', color:done?'#22c55e':active?ACC:T4, border:`1px solid ${done?'rgba(34,197,94,0.2)':active?`${ACC}25`:'transparent'}` }}>
                      {done?'✓':''}{labels[e]}
                    </div>
        ...
        <div style={{ flex:1, maxWidth:'960px', margin:'0 auto', width:'100%', padding:'22px 28px 48px' }}>

          {/* Descripción incidente */}
          <div style={{ padding:'14px 18px', borderRadius:'10px', backgroundColor:`${ACC}06`, border:`1px solid ${ACC}15`, marginBottom:'20px', position:'relative', overflow:'hidden' }}>
          ...
          {/* TIP */}
          {tipActual && (
            <div style={{ padding:'10px 16px', borderRadius:'9px', backgroundColor:'#fffbeb', border:'1px solid #fde68a', marginBottom:'18px', display:'flex', alignItems:'center', gap:'10px' }}>
            ...
            <div className="fade-up">
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:'12px', marginBottom:'24px' }}>
                {incidente.alertas?.map((alerta,i)=>(
                  <div key={i} style={{ padding:'18px 20px', borderRadius:'13px', backgroundColor:CARD, border:`1px solid ${triaje[alerta.id]?SEV[triaje[alerta.id]]+'40':BD}`, boxShadow:'0 2px 8px rgba(0,0,0,0.05)', position:'relative', overflow:'hidden' }}>
                      ...
                      <div style={{ display:'flex', flexDirection:'column', gap:'5px', flexShrink:0 }}>
                        {['CRITICA','ALTA','MEDIA','BAJA'].map(sev=>(
                          <button key={sev} onClick={()=>setTriaje(p=>({...p,[alerta.id]:sev}))}
                            style={{ padding:'7px 12px', borderRadius:'7px', fontSize:'10px', fontWeight:700, fontFamily:'monospace', cursor:'pointer', minWidth:'76px', backgroundColor:triaje[alerta.id]===sev?SEV_BG[sev]:'#f8fafc', border:`1px solid ${triaje[alerta.id]===sev?SEV[sev]:BD}`, color:triaje[alerta.id]===sev?SEV[sev]:T3 }}>
                            {sev}
                          </button>
                        ))}
                      </div>
              ...
              <button onClick={()=>setEtapa('logs')} disabled={!triajeCompleto}
                style={{ width:'100%', padding:'15px', borderRadius:'12px', backgroundColor:triajeCompleto?ACC:BD, border:'none', color:'#fff', fontWeight:700, fontSize:'15px', cursor:triajeCompleto?'pointer':'not-allowed', opacity:triajeCompleto?1:.5 }}>
          ...
          {etapa==='logs' && (
            <div className="fade-up">
                ...
                {incidente.logs?.map((log,i)=>{
                  ...
                  return (
                    <div key={i} className="log-card" onClick={()=>!locked&&toggleLog(i)}
                      style={{ padding:'13px 15px', borderRadius:'11px', backgroundColor:sel?`${ACC}06`:CARD, border:`1px solid ${sel?ACC+'40':BD}`, cursor:locked?'not-allowed':'pointer', opacity:locked?.4:1, position:'relative', overflow:'hidden', boxShadow:'0 1px 4px rgba(0,0,0,0.04)' }}>
              ...
              <button onClick={()=>setEtapa('diagnostico')} disabled={logsEleg.length===0}
                style={{ width:'100%', padding:'15px', borderRadius:'12px', backgroundColor:logsEleg.length>0?ACC:BD, border:'none', color:'#fff', fontWeight:700, fontSize:'15px', cursor:logsEleg.length>0?'pointer':'not-allowed', opacity:logsEleg.length>0?1:.5 }}>
          ...
          {etapa==='diagnostico' && (
            <div className="fade-up">
              ...
              <div style={{ padding:'13px 16px', borderRadius:'10px', backgroundColor:BG, border:`1px solid ${BD}`, marginBottom:'18px' }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'11px', marginBottom:'18px' }}>
                {diagOpts.map((op,i)=>(
                  <button key={i} className="diag-opt" onClick={()=>setDiagEleg(op)}
                    style={{ padding:'18px 16px', borderRadius:'12px', backgroundColor:diagEleg===op?`${ACC}08`:CARD, border:`1.5px solid ${diagEleg===op?ACC:BD}`, color:diagEleg===op?T1:T2, fontSize:'14px', fontWeight:600, cursor:'pointer', textAlign:'left', display:'flex', alignItems:'center', gap:'14px', boxShadow:diagEleg===op?`0 0 16px ${ACC}15`:'0 1px 4px rgba(0,0,0,0.04)' }}>
              ...
              {pista&&(
                <div style={{ padding:'12px 16px', borderRadius:'9px', backgroundColor:'#fffbeb', border:'1px solid #fde68a', marginBottom:'12px' }}>
              ...
              <div style={{ display:'flex', gap:'10px' }}>
                {!pista&&(
                  <button onClick={()=>{ setPistas(p=>p+1); setPista(true); }}
                    style={{ padding:'13px 18px', borderRadius:'11px', backgroundColor:'#fffbeb', border:'1px solid #fde68a', color:'#92400e', fontSize:'13px', fontWeight:600, cursor:'pointer', flexShrink:0 }}>
                    💡 Pista (-1pt)
                  </button>
                )}
                <button onClick={()=>setEtapa('acciones')} disabled={!diagEleg}
                  style={{ flex:1, padding:'13px', borderRadius:'11px', backgroundColor:diagEleg?ACC:BD, border:'none', color:'#fff', fontWeight:700, fontSize:'15px', cursor:diagEleg?'pointer':'not-allowed', opacity:diagEleg?1:.5 }}>
          ...
          {etapa==='acciones' && (
            <div className="fade-up">
                ...
                {ACCIONES.map(({id,label,icon,desc})=>{
                  ...
                  return (
                    <button key={id} className="accion-btn" onClick={()=>toggleAccion(id)}
                      style={{ padding:'15px 16px', borderRadius:'12px', backgroundColor:sel?`${ACC}06`:CARD, border:`1.5px solid ${sel?ACC+'50':BD}`, color:T1, cursor:'pointer', textAlign:'left', display:'flex', alignItems:'center', gap:'12px', boxShadow:'0 1px 4px rgba(0,0,0,0.04)' }}>
              ...
              <button onClick={()=>setEtapa('justificacion')} disabled={accionesEl.length===0}
                style={{ width:'100%', padding:'15px', borderRadius:'12px', backgroundColor:accionesEl.length>0?ACC:BD, border:'none', color:'#fff', fontWeight:700, fontSize:'15px', cursor:accionesEl.length>0?'pointer':'not-allowed', opacity:accionesEl.length>0?1:.5 }}>
          ...
          {etapa==='justificacion' && (
            <div className="fade-up">
              ...
              <div style={{ padding:'16px 18px', borderRadius:'12px', backgroundColor:CARD, border:`1px solid ${BD}`, marginBottom:'14px', boxShadow:'0 1px 4px rgba(0,0,0,0.04)' }}>
              ...
              <textarea value={justif} onChange={e=>setJustif(e.target.value)}
                ...
                style={{ width:'100%', height:'165px', padding:'15px', borderRadius:'12px', fontFamily:'monospace', fontSize:'13px', color:T1, backgroundColor:CARD, border:`1.5px solid ${justif.trim()?ACC+'40':BD}`, resize:'vertical', lineHeight:1.8, marginBottom:'10px', display:'block', boxShadow:'0 1px 4px rgba(0,0,0,0.04)' }}
              />
              <div style={{ display:'flex', justifyContent:'space-between', marginBottom:'12px' }}>
