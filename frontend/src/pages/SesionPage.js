...
const TIPS = {
  ...
  analisis: [
    '💡 Las respuestas deben ser concretas — IPs, nombres de proceso, IDs de técnica.',
    '💡 Para técnicas MITRE usa el formato T1XXX o el nombre exacto.',
    '💡 En la acción de contención, sé específico: incluye el nombre del host.',
    '💡 Busca en los logs el IOC más relevante — normalmente es la primera alerta CRÍTICA.',
  ],
};
...
const Q_ICONS = { ip:'🌐', hash:'#️⃣', proceso:'⚙️', tecnica:'🎯', accion:'⚡', texto:'📝' };
const Q_COLORS = ['#ef4444','#f97316','#8b5cf6','#10b981'];
...
const ParticlesBg = () => {
  ...
  useEffect(() => {
    ...
    const pts = Array.from({length:38},()=>({ x:Math.random()*w, y:Math.random()*h, vx:(Math.random()-.5)*.3, vy:(Math.random()-.5)*.3, r:Math.random()*1.6+.6, o:Math.random()*.12+.03 }));
    let raf;
    const draw = () => {
      ...
      pts.forEach((a,i)=>pts.slice(i+1).forEach(b=>{ const d=Math.hypot(a.x-b.x,a.y-b.y); if(d<140){ ctx.beginPath(); ctx.moveTo(a.x,a.y); ctx.lineTo(b.x,b.y); ctx.strokeStyle=`rgba(79,70,229,${(1-d/140)*.04})`; ctx.lineWidth=.4; ctx.stroke(); } }));
...
const Timer = ({ segundos, limite }) => {
  const pct=Math.max(0,segundos/limite), critico=segundos<45, warning=segundos<120;
...
const ModalSalir = ({ onConfirm, onCancel }) => (
  <div style={{ position:'fixed',inset:0,backgroundColor:'rgba(15,23,42,0.45)',zIndex:9999,display:'flex',alignItems:'center',justifyContent:'center',fontFamily:"'Inter',sans-serif",backdropFilter:'blur(4px)' }}>
    <div style={{ maxWidth:'400px',width:'90%',padding:'36px',borderRadius:'20px',backgroundColor:CARD,border:`1px solid ${BD}`,boxShadow:'0 24px 64px rgba(0,0,0,0.18)',textAlign:'center' }}>
      ...
      <p style={{ fontSize:'14px',color:T3,lineHeight:1.7,marginBottom:'28px' }}>Si sales ahora <strong style={{ color:'#ef4444' }}>perderás 10 copas</strong> por abandonar.</p>
      <div style={{ display:'flex',gap:'12px' }}>
        <button onClick={onCancel} style={{ flex:1,padding:'13px',borderRadius:'10px',backgroundColor:BG,border:`1px solid ${BD}`,color:T2,fontSize:'14px',fontWeight:600,cursor:'pointer' }}>Continuar</button>
        <button onClick={onConfirm} style={{ flex:1,padding:'13px',borderRadius:'10px',backgroundColor:'rgba(239,68,68,0.07)',border:'1px solid rgba(239,68,68,0.25)',color:'#dc2626',fontSize:'14px',fontWeight:700,cursor:'pointer' }}>Salir (-10 🏆)</button>
      </div>
    </div>
  </div>
);
...
export default function SesionPage() {
  ...
  const [respuestasQ,  setRespuestasQ]  = useState({ Q1:'', Q2:'', Q3:'', Q4:'' });
  const [qActiva,      setQActiva]      = useState(null);
  ...
  // Tiempos reducidos ~25% respecto al original
  const TIEMPOS = { Bronce:900, Plata:600, Oro:420, Diamante:300 };
  ...
  const limiteSegundos = TIEMPOS[tierArena]||900;

  useEffect(()=>{ axios.get(`${API}/api/me`,{headers:{Authorization:`Bearer ${token}`}}).then(r=>setUserData(r.data)).catch(()=>{}); },[]);
  ...
  const resetEtapas = ()=>{ setEtapa('triaje'); setTriaje({}); setLogsEleg([]); setDiagEleg(null); setAccionesEl([]); setRespuestasQ({Q1:'',Q2:'',Q3:'',Q4:''}); setQActiva(null); setPistas(0); setPista(false); };

  const iniciarCountdown = ()=>{
    setCargandoCountdown(40);
    countdownRef.current = setInterval(()=>{ setCargandoCountdown(p=>{ if(p<=1){clearInterval(countdownRef.current);return 0;} return p-1; }); },1000);
  };
  ...
  const generar = async ()=>{
    ...
    try {
      ...
      setSesion(r.data); setTiempoRestante(TIEMPOS[tierArena]||900); setFase('briefing');
  ...
  const intentarSalir = ()=>{ if(fase==='activa'||fase==='feedback') setMostrarModalSalir(true); else navigate('/dashboard'); };
  ...
  const enviar = async (timeout=false)=>{
    ...
    const qStr = `Q1:${respuestasQ.Q1}|Q2:${respuestasQ.Q2}|Q3:${respuestasQ.Q3}|Q4:${respuestasQ.Q4}`;
    const txt=`TRIAJE:${JSON.stringify(triaje)}|LOGS:${logsEleg.join(',')}|DIAG:${diagEleg}|ACCIONES:${accionesEl.join(',')}|${qStr}`;
    try {
  ...
  const setQ = (key, val) => setRespuestasQ(p=>({...p,[key]:val}));

  const ETAPAS   = ['triaje','logs','diagnostico','acciones','analisis'];
  const etapaIdx = ETAPAS.indexOf(etapa);
  const triajeCompleto = Object.keys(triaje).length>=(incidente?.alertas?.length||0);
  const analisisCompleto = Object.values(respuestasQ).filter(v=>v.trim()).length >= 2;
  ...
  const css = `
    ...
    @keyframes htbIn{from{opacity:0;transform:translateY(6px);}to{opacity:1;transform:none;}}
    ...
    .htb-input:focus{outline:none;border-color:${ACC}!important;box-shadow:0 0 0 3px ${ACC}15!important;}
    .htb-card:hover{border-color:${ACC}40!important;}
  ...
  if (contando) return (<><style>{css}</style><CuentaAtras onFin={()=>{ setContando(false); resetEtapas(); setFase('activa'); }}/></>);
          ...
          <div style={{ maxWidth:'680px',margin:'0 auto',padding:'44px 28px' }}>
            <div className="fade-up">
              <div style={{ textAlign:'center',marginBottom:'36px' }}>
                ...
                <p style={{ fontSize:'15px',color:T3,lineHeight:1.75,maxWidth:'460px',margin:'0 auto' }}>Escenario real generado por IA. Investiga, diagnostica y responde como analista profesional.</p>
              </div>

              <div style={{ display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'12px',marginBottom:'24px' }}>
                {[
                  {label:'Arena',   value:arena,   color:ac},
                  {label:'Tiempo',  value:{Bronce:'15 min',Plata:'10 min',Oro:'7 min',Diamante:'5 min'}[tierArena]||'15 min', color:'#22c55e'},
              ...
              <div style={{ padding:'22px 24px',borderRadius:'16px',backgroundColor:'rgba(255,255,255,0.85)',border:`1px solid ${BD}`,marginBottom:'24px',boxShadow:'0 2px 8px rgba(0,0,0,0.05)' }}>
                ...
                <div style={{ display:'flex',flexDirection:'column',gap:'9px' }}>
                  {[
                    ...
                    {n:'04',t:'Respuesta',   d:'Elige acciones de contención — el orden importa',   c:'#22c55e'},
                    {n:'05',t:'Análisis HTB',d:'4 preguntas técnicas con respuesta exacta',         c:ACC},
                  ].map((m,i)=>(
    ...
    <><style>{css}</style>
      <div style={{ minHeight:'100vh',background:'linear-gradient(150deg,#f0f4ff 0%,#f8f9ff 40%,#f5f0ff 100%)',display:'flex',flexDirection:'column',fontFamily:"'Inter',sans-serif",position:'relative' }}>
        ...
        <nav style={{ position:'relative',zIndex:10,height:'56px',display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0 36px',backgroundColor:'rgba(255,255,255,0.92)',borderBottom:`1px solid ${BD}`,backdropFilter:'blur(20px)' }}>
          ...
          <button onClick={cancelarCarga} style={{ background:'none',border:`1px solid ${BD}`,color:T3,padding:'6px 14px',borderRadius:'8px',fontSize:'12px',cursor:'pointer' }}>✕ Cancelar</button>
        </nav>
      ...
      <div style={{ minHeight:'100vh',background:'linear-gradient(150deg,#f0f4ff 0%,#f8f9ff 40%,#f5f0ff 100%)',fontFamily:"'Inter',sans-serif",display:'flex',flexDirection:'column',position:'relative' }}>
        ...
        <div style={{ flex:1,display:'flex',alignItems:'center',justifyContent:'center',padding:'32px 24px',position:'relative',zIndex:1 }}>
          <div className="fade-up" style={{ maxWidth:'600px',width:'100%' }}>
            <div style={{ padding:'40px',borderRadius:'22px',backgroundColor:'rgba(255,255,255,0.95)',border:`1px solid ${BD}`,boxShadow:'0 12px 48px rgba(0,0,0,0.08)',position:'relative',overflow:'hidden' }}>
  ...
  if (fase==='evaluando') return (
    <><style>{css}</style>
      <div style={{ minHeight:'100vh',background:'linear-gradient(150deg,#f0f4ff 0%,#f8f9ff 40%,#f5f0ff 100%)',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:"'Inter',sans-serif",position:'relative' }}>
        ...
        <div style={{ textAlign:'center',position:'relative',zIndex:1 }}>
          <img src="/logosoc.png" alt="Cargando" style={{ width:'64px',height:'64px',animation:'spinLogo 1s linear infinite',marginBottom:'24px' }}/>
          <p style={{ color:ACC,fontSize:'15px',fontWeight:700,letterSpacing:'2px',marginBottom:'8px',fontFamily:'monospace' }}>EVALUANDO RESPUESTA...</p>
          <p style={{ color:T3,fontSize:'13px' }}>Analizando decisiones · Calculando puntuación</p>
        </div>
      </div>
    </>
  );
  ...
  if (fase==='feedback' && evaluacion) {
    ...
    const pregsResultado = evaluacion.preguntas_resultado||[];
    const pregsOriginales = incidente?.preguntas_analisis||[];
    return (
      <><style>{css}</style>
        ...
        <div style={{ minHeight:'100vh',background:'linear-gradient(150deg,#f0f4ff 0%,#f8f9ff 40%,#f5f0ff 100%)',fontFamily:"'Inter',sans-serif",position:'relative' }}>
          ...
          <div style={{ position:'relative',zIndex:1 }}>
            ...
            <div style={{ maxWidth:'760px',margin:'0 auto',padding:'36px 28px' }}>
              <div className="fade-up">
                <div style={{ padding:'36px',borderRadius:'22px',backgroundColor:CARD,border:`1.5px solid ${sc}20`,boxShadow:'0 8px 40px rgba(0,0,0,0.07)',marginBottom:'16px',position:'relative',overflow:'hidden' }}>
                  ...
                  {/* Resultados HTB */}
                  {pregsOriginales.length > 0 && (
                    <div style={{ marginBottom:'16px' }}>
                      <p style={{ fontSize:'10px',color:T4,fontWeight:700,letterSpacing:'1.5px',marginBottom:'10px',fontFamily:'monospace' }}>RESPUESTAS HTB:</p>
                      <div style={{ display:'flex',flexDirection:'column',gap:'8px' }}>
                        {pregsOriginales.map((preg,i)=>{
                          const res = pregsResultado[i];
                          const correcto = res?.correcto;
                          const miResp = respuestasQ[`Q${i+1}`];
                          return (
                            <div key={i} style={{ display:'flex',alignItems:'flex-start',gap:'10px',padding:'11px 14px',borderRadius:'10px',backgroundColor:correcto?'#f0fdf4':'#fef2f2',border:`1px solid ${correcto?'#bbf7d0':'#fecaca'}` }}>
                              <div style={{ width:'22px',height:'22px',borderRadius:'6px',flexShrink:0,backgroundColor:correcto?'#10b981':'#ef4444',display:'flex',alignItems:'center',justifyContent:'center',marginTop:'1px' }}>
                                <span style={{ color:'#fff',fontSize:'11px',fontWeight:800 }}>{correcto?'✓':'✗'}</span>
                              </div>
                              <div style={{ flex:1,minWidth:0 }}>
                                <p style={{ fontSize:'12px',fontWeight:700,color:T1,marginBottom:'3px' }}>{preg.pregunta}</p>
                                <p style={{ fontSize:'11px',color:T3,fontFamily:'monospace',marginBottom:'2px' }}>Tu respuesta: <span style={{ color:correcto?'#059669':'#ef4444',fontWeight:600 }}>{miResp||'(sin respuesta)'}</span></p>
                                {!correcto && <p style={{ fontSize:'11px',color:'#059669',fontFamily:'monospace' }}>Correcta: <strong>{preg.respuesta_correcta}</strong></p>}
                              </div>
                              {res && <span style={{ fontSize:'11px',fontWeight:700,color:correcto?'#059669':'#94a3b8',fontFamily:'monospace',flexShrink:0 }}>{res.puntos||0}/1.5</span>}
                            </div>
                          );
                        })}
                      </div>
  ...
  const preguntasHTB   = incidente.preguntas_analisis||[];
        ...
        <div style={{ position:'relative',zIndex:10,height:'52px',display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0 28px',backgroundColor:'rgba(255,255,255,0.92)',backdropFilter:'blur(20px)',borderBottom:`1px solid ${BD}`,flexShrink:0,boxShadow:'0 1px 8px rgba(0,0,0,0.06)' }}>
          <div style={{ display:'flex',alignItems:'center',gap:'14px' }}>
            ...
            <div style={{ display:'flex',gap:'4px',alignItems:'center' }}>
              {ETAPAS.map((e,i)=>{
                const labels={triaje:'TRIAJE',logs:'LOGS',diagnostico:'DIAG',acciones:'RESP',analisis:'HTB'};
        ...
        <div style={{ flex:1,maxWidth:'960px',margin:'0 auto',width:'100%',padding:'22px 28px 48px',position:'relative',zIndex:1 }}>
          ...
          {etapa==='acciones' && (
            <div className="fade-up">
              ...
              <button onClick={()=>setEtapa('analisis')} disabled={accionesEl.length===0}
            ...
            <div className="fade-up">
              <div style={{ marginBottom:'20px' }}>
                <div style={{ display:'flex',alignItems:'center',gap:'10px',marginBottom:'8px' }}>
                  <h2 style={{ fontSize:'21px',fontWeight:800,color:T1 }}>🔬 Análisis Técnico</h2>
                  <span style={{ fontSize:'10px',fontWeight:700,padding:'3px 8px',borderRadius:'5px',backgroundColor:'rgba(79,70,229,0.08)',color:ACC,border:`1px solid ${ACC}20`,fontFamily:'monospace' }}>HACK THE BOX STYLE</span>
                </div>
                <p style={{ fontSize:'13px',color:T3 }}>Responde con precisión. Cada pregunta tiene una respuesta exacta extraíble de los datos del incidente.</p>
              </div>

              {/* Resumen de decisiones */}
              <div style={{ padding:'12px 16px',borderRadius:'10px',backgroundColor:'rgba(255,255,255,0.85)',border:`1px solid ${BD}`,marginBottom:'20px',backdropFilter:'blur(8px)',display:'flex',gap:'20px',flexWrap:'wrap' }}>
                <div>
                  <span style={{ fontSize:'10px',color:T4,fontFamily:'monospace',letterSpacing:'1px' }}>DIAGNÓSTICO · </span>
                  <span style={{ fontSize:'12px',color:ACC,fontWeight:700 }}>{diagEleg}</span>
                </div>
                <div>
                  <span style={{ fontSize:'10px',color:T4,fontFamily:'monospace',letterSpacing:'1px' }}>ACCIONES · </span>
                  <span style={{ fontSize:'12px',color:T2,fontWeight:600 }}>{accionesEl.length} seleccionadas</span>
                </div>
              </div>

              {/* Las 4 preguntas HTB */}
              <div style={{ display:'flex',flexDirection:'column',gap:'14px',marginBottom:'24px' }}>
                {preguntasHTB.map((preg,i)=>{
                  const qKey   = `Q${i+1}`;
                  const color  = Q_COLORS[i]||ACC;
                  const icon   = Q_ICONS[preg.tipo]||'📝';
                  const activa = qActiva===i;
                  const tieneRespuesta = respuestasQ[qKey]?.trim().length > 0;

                  return (
                    <div key={i} className="htb-card"
                      style={{ borderRadius:'14px',backgroundColor:'rgba(255,255,255,0.92)',border:`1.5px solid ${tieneRespuesta?color+'50':activa?color+'40':BD}`,overflow:'hidden',boxShadow:activa?`0 4px 20px ${color}12`:'0 2px 8px rgba(0,0,0,0.05)',backdropFilter:'blur(8px)',animation:`htbIn .25s ease ${i*.08}s both` }}>

                      {/* Header pregunta */}
                      <div style={{ padding:'14px 18px',borderBottom:`1px solid ${tieneRespuesta?color+'20':BD}`,background:tieneRespuesta?`${color}04`:activa?`${color}03`:'transparent',display:'flex',alignItems:'flex-start',gap:'12px' }}>
                        <div style={{ width:'32px',height:'32px',borderRadius:'9px',backgroundColor:`${color}12`,border:`1px solid ${color}20`,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,fontSize:'16px' }}>
                          {icon}
                        </div>
                        <div style={{ flex:1 }}>
                          <div style={{ display:'flex',alignItems:'center',gap:'8px',marginBottom:'4px' }}>
                            <span style={{ fontSize:'10px',fontWeight:800,color:color,fontFamily:'monospace',letterSpacing:'1px' }}>TAREA {i+1}</span>
                            <span style={{ fontSize:'10px',padding:'1px 6px',borderRadius:'4px',backgroundColor:`${color}10`,color:color,border:`1px solid ${color}15`,fontFamily:'monospace' }}>{preg.tipo?.toUpperCase()||'TEXTO'}</span>
                            {tieneRespuesta && <span style={{ fontSize:'10px',padding:'1px 6px',borderRadius:'4px',backgroundColor:'rgba(34,197,94,0.1)',color:'#16a34a',border:'1px solid rgba(34,197,94,0.2)',fontFamily:'monospace' }}>✓ RESPONDIDA</span>}
                          </div>
                          <p style={{ fontSize:'14px',fontWeight:700,color:T1,lineHeight:1.5 }}>{preg.pregunta}</p>
                        </div>
                      </div>
                      ...
                      <div style={{ padding:'14px 18px' }}>
                        <div style={{ position:'relative' }}>
                          <input
                            className="htb-input"
                            type="text"
                            value={respuestasQ[qKey]}
                            onChange={e=>setQ(qKey,e.target.value)}
                            onFocus={()=>setQActiva(i)}
                            onBlur={()=>setQActiva(null)}
                            placeholder={preg.placeholder||`Respuesta ${i+1}...`}
                            style={{ width:'100%',padding:'11px 14px',paddingRight:tieneRespuesta?'40px':'14px',borderRadius:'8px',border:`1.5px solid ${tieneRespuesta?color+'60':activa?color+'50':BD}`,backgroundColor:tieneRespuesta?`${color}04`:'#fff',color:T1,fontSize:'13px',fontFamily:'monospace',boxSizing:'border-box',outline:'none' }}
                          />
                          {tieneRespuesta && (
                            <div style={{ position:'absolute',right:'12px',top:'50%',transform:'translateY(-50%)',width:'18px',height:'18px',borderRadius:'50%',backgroundColor:color,display:'flex',alignItems:'center',justifyContent:'center' }}>
                        ...
                        <div style={{ marginTop:'8px',display:'flex',alignItems:'flex-start',gap:'6px' }}>
                          ...
                          <span style={{ fontSize:'11px',color:'#92400e',lineHeight:1.5 }}>{preg.pista}</span>
                        </div>
                      </div>
                    </div>
                  );
              ...
              <div style={{ marginBottom:'14px' }}>
                <div style={{ display:'flex',justifyContent:'space-between',fontSize:'12px',color:T4,marginBottom:'6px',fontFamily:'monospace' }}>
                  <span>Progreso</span>
                  <span>{Object.values(respuestasQ).filter(v=>v.trim()).length}/4 respondidas</span>
                </div>
                <div style={{ height:'4px',backgroundColor:BD,borderRadius:'2px',overflow:'hidden' }}>
                  <div style={{ height:'100%',borderRadius:'2px',width:`${(Object.values(respuestasQ).filter(v=>v.trim()).length/4)*100}%`,background:`linear-gradient(90deg,${ACC},#818cf8)`,transition:'width .4s ease' }}/>
                </div>
              </div>

              <button onClick={()=>enviar()} disabled={!analisisCompleto}
                style={{ width:'100%',padding:'17px',borderRadius:'12px',backgroundColor:analisisCompleto?ACC:T4,border:'none',color:'#fff',fontWeight:800,fontSize:'16px',cursor:analisisCompleto?'pointer':'not-allowed',opacity:analisisCompleto?1:.5,display:'flex',alignItems:'center',justifyContent:'center',gap:'12px',boxShadow:analisisCompleto?`0 4px 24px ${ACC}25`:'none' }}>
              ...
              {!analisisCompleto && <p style={{ textAlign:'center',fontSize:'11px',color:T4,marginTop:'8px',fontFamily:'monospace' }}>Responde al menos 2 preguntas para continuar</p>}
            </div>
          )}
        </div>
      </div>
