...
const AVATAR_SEEDS = [
  { id:'felix',   label:'Felix',   free:true  },
  { id:'zoe',     label:'Zoe',     free:true  },
  { id:'max',     label:'Max',     free:true  },
  { id:'luna',    label:'Luna',    free:true  },
  { id:'alex',    label:'Alex',    free:true  },
  { id:'kai',     label:'Kai',     free:true  },
  { id:'nova',    label:'Nova',    free:true  },
  { id:'rex',     label:'Rex',     free:true  },
  { id:'cipher',  label:'Cipher',  free:false, requireTier:3 },
  { id:'phantom', label:'Phantom', free:false, requireTier:4 },
  { id:'ghost',   label:'Ghost',   free:false, requireTier:5 },
  { id:'viper',   label:'Viper',   free:false, requireTier:6 },
  { id:'legend',  label:'Legend',  free:false, requireTier:7 },
  { id:'apex',    label:'Apex',    free:false, requireTier:8 },
];

const BG_COLORS = [
  { id:'b6e3f4', label:'Cielo',    free:true  },
  { id:'ffdfbf', label:'Arena',    free:true  },
  { id:'d1d4f9', label:'Lavanda',  free:true  },
  { id:'c0aede', label:'Lila',     free:true  },
  { id:'ffd5dc', label:'Rosa',     free:true  },
  { id:'d4f5e9', label:'Menta',    free:true  },
  { id:'1e1b4b', label:'Noche',    free:false, requireTier:3 },
  { id:'064e3b', label:'Matrix',   free:false, requireTier:4 },
  { id:'0a0118', label:'Galaxia',  free:false, requireTier:5 },
  { id:'0f0f1a', label:'Neon',     free:false, requireTier:6 },
];

const HAIR_COLORS = [
  { id:'0e0e0e', label:'Negro',    free:true },
  { id:'6d4c41', label:'Castaño',  free:true },
  { id:'f59e0b', label:'Rubio',    free:true },
  { id:'ef4444', label:'Rojo',     free:true },
  { id:'8b5cf6', label:'Morado',   free:false, requireTier:3 },
  { id:'06b6d4', label:'Cian',     free:false, requireTier:4 },
  { id:'10b981', label:'Verde',    free:false, requireTier:5 },
  { id:'f0f0f0', label:'Blanco',   free:false, requireTier:6 },
];

const SKIN_TONES = [
  { id:'fdbcb4', label:'Clara',       free:true },
  { id:'f5cba7', label:'Melocotón',   free:true },
  { id:'c68642', label:'Media',       free:true },
  { id:'8d5524', label:'Bronceada',   free:true },
  { id:'4a2912', label:'Oscura',      free:true },
];
...
// Genera SVG string con DiceBear
function generateAvatarSvg(av = {}) {
  const cfg = { ...defaultAvatar, ...av };
  try {
...
// Componente avatar que renderiza el SVG inline
const AvatarImg = ({ avatar = {}, size = 80, style = {} }) => {
  const svgStr = useMemo(() => generateAvatarSvg(avatar), [
    avatar.seed, avatar.bg, avatar.hairColor, avatar.skinColor,
  ]);
  if (!svgStr) return (
    <div style={{ width: size, height: size, borderRadius: '50%', backgroundColor: '#eef2ff', flexShrink: 0, ...style }}/>
  );
  return (
    <div
      style={{ width: size, height: size, borderRadius: '50%', overflow: 'hidden', flexShrink: 0, ...style }}
      dangerouslySetInnerHTML={{ __html: svgStr }}
    />
  );
};

// Mini preview para selector de items
const AvatarPreviewSmall = ({ avatar, overrides = {}, size = 52 }) => {
  const merged = { ...defaultAvatar, ...avatar, ...overrides };
  return <AvatarImg avatar={merged} size={size}/>;
};
...
export default function PerfilPage() {
  ...
  const [avatarTab, setAvatarTab] = useState('seed');
  ...
  const AvatarEditor = () => {
    const tabConfig = {
      seed: {
        ...
        items: AVATAR_SEEDS,
        renderItem: (item) => (
          <AvatarPreviewSmall avatar={avatar} overrides={{seed: item.id}} size={56}/>
        ),
        onSelect: (item) => saveAvatar({...avatar, seed: item.id}),
        isSelected: (item) => avatar.seed === item.id,
      },
      bg: {
        label: 'Fondo',
        items: BG_COLORS,
        renderItem: (item) => (
          <div style={{width:48,height:48,borderRadius:'50%',backgroundColor:`#${item.id}`,border:'2px solid #e2e8f0'}}/>
        ),
        onSelect: (item) => saveAvatar({...avatar, bg: item.id}),
        isSelected: (item) => avatar.bg === item.id,
      },
      hair: {
        label: 'Pelo',
        items: HAIR_COLORS,
        renderItem: (item) => (
          <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:'4px'}}>
            <div style={{width:32,height:32,borderRadius:'50%',backgroundColor:`#${item.id}`,border:'2px solid #e2e8f0'}}/>
            <AvatarPreviewSmall avatar={avatar} overrides={{hairColor: item.id}} size={36}/>
          </div>
        ),
        onSelect: (item) => saveAvatar({...avatar, hairColor: item.id}),
        isSelected: (item) => avatar.hairColor === item.id,
      },
      skin: {
        label: 'Piel',
        items: SKIN_TONES,
        renderItem: (item) => (
          <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:'4px'}}>
            <div style={{width:36,height:36,borderRadius:'50%',backgroundColor:`#${item.id}`,border:'2px solid #e2e8f0'}}/>
            <AvatarPreviewSmall avatar={avatar} overrides={{skinColor: item.id}} size={36}/>
          </div>
        ),
        onSelect: (item) => saveAvatar({...avatar, skinColor: item.id}),
        isSelected: (item) => avatar.skinColor === item.id,
      },
    };

    const current = tabConfig[avatarTab];
      ...
      <div className="fade-up">
        <div style={{display:'grid',gridTemplateColumns:'260px 1fr',gap:'24px',alignItems:'start'}}>
          ...
          <div style={{padding:'28px 24px',borderRadius:'20px',backgroundColor:'#fff',border:'1px solid #e8eaf0',boxShadow:'0 4px 20px rgba(0,0,0,0.07)',textAlign:'center',position:'sticky',top:'80px'}}>
            <p style={{fontSize:'10px',color:'#94a3b8',fontWeight:700,letterSpacing:'2px',marginBottom:'20px',fontFamily:'monospace'}}>PREVIEW EN VIVO</p>
            <div className="avatar-float" style={{display:'flex',justifyContent:'center',marginBottom:'16px'}}>
              <AvatarImg avatar={avatar} size={120}/>
            </div>
            <p style={{fontSize:'15px',fontWeight:800,color:'#0f172a',marginBottom:'3px'}}>{userData?.nombre}</p>
            ...
            <div style={{padding:'12px',borderRadius:'12px',backgroundColor:'#f8fafc',border:'1px solid #e2e8f0',textAlign:'left'}}>
              ...
              <div style={{display:'flex',alignItems:'center',gap:'10px',padding:'8px 10px',borderRadius:'8px',backgroundColor:'#fff',border:'1px solid #e8eaf0'}}>
                ...
                <div style={{flex:1}}>
                  <p style={{fontSize:'12px',fontWeight:700,color:'#0f172a',marginBottom:'1px'}}>{userData?.nombre}</p>
            ...
            <div style={{marginTop:'12px',padding:'10px 12px',borderRadius:'10px',backgroundColor:`${tierData?.color}08`,border:`1px solid ${tierData?.color}20`}}>
              <p style={{fontSize:'10px',color:tierData?.color,fontWeight:700}}>{tierData?.name} — T{tierActual}</p>
              <p style={{fontSize:'9px',color:'#94a3b8',marginTop:'2px'}}>Sube de tier para desbloquear más</p>
            </div>
          </div>
          ...
          <div style={{padding:'24px',borderRadius:'20px',backgroundColor:'#fff',border:'1px solid #e8eaf0',boxShadow:'0 4px 20px rgba(0,0,0,0.07)'}}>
            ...
            <div style={{display:'flex',gap:'6px',marginBottom:'20px',flexWrap:'wrap'}}>
              {Object.entries(tabConfig).map(([key,cfg])=>(
                <button key={key} onClick={()=>setAvatarTab(key)}
                  style={{padding:'7px 16px',borderRadius:'9px',border:`1px solid ${avatarTab===key?ACC:'#e2e8f0'}`,backgroundColor:avatarTab===key?ACC:'#fff',color:avatarTab===key?'#fff':'#64748b',fontSize:'12px',fontWeight:600,cursor:'pointer'}}>
                  {cfg.label}
                </button>
              ))}
            </div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'10px'}}>
              {current.items.map((item,i)=>{
                const isSelected = current.isSelected(item);
                const locked     = !canUnlock(item);
                return (
                  <div key={i}
                    className={locked ? '' : 'item-card'}
                    onClick={()=>{ if (!locked) current.onSelect(item); }}
                    style={{
                      padding:'14px 10px',
                      borderRadius:'14px',
                      border:`2px solid ${isSelected?ACC:locked?'#e8eaf0':'#f1f5f9'}`,
                      backgroundColor: isSelected?`${ACC}06`:locked?'#f8fafc':'#fff',
                      textAlign:'center',
                    ...
                    {isSelected && (
                      <div style={{position:'absolute',top:7,left:7,width:'16px',height:'16px',borderRadius:'50%',backgroundColor:ACC,display:'flex',alignItems:'center',justifyContent:'center',zIndex:2}}>
                        <span style={{fontSize:'8px',color:'#fff',fontWeight:700}}>✓</span>
                      </div>
                    )}
                    {locked && (
                      <div style={{position:'absolute',top:7,right:7,fontSize:'12px',zIndex:2}}>🔒</div>
                    )}
                    <div style={{display:'flex',justifyContent:'center',alignItems:'center',marginBottom:'8px',height:'58px'}}>
                      {current.renderItem(item)}
                    </div>
                    <p style={{fontSize:'11px',color:isSelected?ACC:locked?'#cbd5e1':'#374151',fontWeight:isSelected?700:500,lineHeight:1.2,margin:0}}>{item.label}</p>
                    {item.requireTier && (
                      <p style={{fontSize:'9px',color:locked?'#cbd5e1':'#f59e0b',fontWeight:600,marginTop:'3px',margin:'3px 0 0'}}>
                        {locked ? `T${item.requireTier} requerido` : `T${item.requireTier}+`}
                      </p>
                    )}
                  </div>
                );
            ...
            <div style={{marginTop:'16px',padding:'12px 16px',borderRadius:'12px',backgroundColor:'#fffbeb',border:'1px solid #fde68a',display:'flex',gap:'10px',alignItems:'flex-start'}}>
              <span style={{fontSize:'15px',flexShrink:0}}>💡</span>
              <p style={{fontSize:'11px',color:'#92400e',lineHeight:1.6,margin:0}}>
                Los items con 🔒 se desbloquean al subir de Tier jugando sesiones SOC y completando laboratorios.
              </p>
            </div>
          </div>
        </div>
          ...
          <div style={{display:'flex',marginBottom:'28px',borderBottom:'1px solid #e8eaf0'}}>
            {TABS.map(tab=>(
              <button key={tab.id} className="tab-btn" onClick={()=>setVista(tab.id)}
                ...
                {tab.id==='avatar' && <AvatarImg avatar={avatar} size={22}/>}
                {tab.label}
              </button>
            ))}
          </div>
            ...
            <div className="fade-up">
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px'}}>
                ...
                <button onClick={()=>setVista('avatar')} style={{padding:'13px',borderRadius:'10px',backgroundColor:'#f8fafc',border:'1px solid #e2e8f0',color:'#475569',fontSize:'13px',cursor:'pointer',fontWeight:600,display:'flex',alignItems:'center',justifyContent:'center',gap:'8px'}}>
                  <AvatarImg avatar={avatar} size={20}/> Editar avatar
                </button>
              </div>
            </div>
          )}
