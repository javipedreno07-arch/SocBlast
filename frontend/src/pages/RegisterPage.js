...
const RegisterPage = () => {
  ...
  const css = `
    ...
    .rol-btn:hover{border-color:rgba(37,100,241,0.3) !important;}
      ...
      <div style={{ minHeight: '100vh', backgroundColor: BG, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', fontFamily: "'Inter',-apple-system,sans-serif" }}>
        <div style={{ width: '100%', maxWidth: '400px' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: '32px' }}>
            ...
            <p style={{ fontSize: '13px', color: T2 }}>Crea tu cuenta gratis</p>
          </div>
          ...
          <div style={{ backgroundColor: CARD, border: `1px solid ${BD}`, borderRadius: '14px', padding: '28px', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ marginBottom: '16px' }}>
              <label style={{ fontSize: '10px', color: T3, fontWeight: 700, letterSpacing: '1.5px', fontFamily: 'monospace', display: 'block', marginBottom: '8px' }}>SOY UN...</label>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                {[{ id: 'analista', label: '🛡️ Analista SOC', sub: 'Quiero entrenar' }, { id: 'empresa', label: '🏢 Empresa', sub: 'Busco talento' }].map(r => (
                  <button key={r.id} className="rol-btn" onClick={() => setForm({ ...form, rol: r.id })}
                    style={{ padding: '12px', borderRadius: '8px', backgroundColor: form.rol === r.id ? `${ACC}20` : BG, border: `1px solid ${form.rol === r.id ? ACC : BD}`, color: form.rol === r.id ? ACC : T2, fontSize: '12px', fontWeight: 600, cursor: 'pointer', textAlign: 'center' }}>
                    <div style={{ fontSize: '20px', marginBottom: '4px' }}>{r.label.split(' ')[0]}</div>
                    <div>{r.label.split(' ').slice(1).join(' ')}</div>
                    <div style={{ fontSize: '10px', opacity: 0.6, marginTop: '2px' }}>{r.sub}</div>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '16px' }}>
              <div>
                <label style={{ fontSize: '10px', color: T3, fontWeight: 700, letterSpacing: '1.5px', fontFamily: 'monospace', display: 'block', marginBottom: '6px' }}>NOMBRE</label>
