  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #f0f4ff 0%, #fafbff 50%, #f0f7ff 100%)', display: 'flex', fontFamily: "'Inter',-apple-system,sans-serif" }}>
      <style>{`
        ...
        .rol-card:hover{border-color:#93c5fd !important}
        .lnk:hover{text-decoration:underline}
      `}</style>

      {/* LEFT PANEL */}
      <div style={{ flex: 1, background: '#1e3a8a', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '48px', minHeight: '100vh', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative' }}>
          <div style={{ fontSize: 11, color: '#93c5fd', letterSpacing: '2px', fontWeight: 600, marginBottom: 16 }}>ÚNETE GRATIS</div>
          ...
          <p style={{ fontSize: 15, color: '#bfdbfe', lineHeight: 1.7, maxWidth: 340 }}>Más de 2.400 analistas ya entrenan en SocBlast. Únete y demuestra tu nivel.</p>
          <div style={{ marginTop: 40, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            {[{ v: '2.400+', l: 'Analistas activos' }, { v: '8', l: 'Módulos training' }, { v: '4', l: 'Arenas competitivas' }, { v: '98%', l: 'Satisfacción' }].map((s, i) => (
              <div key={i} style={{ padding: '16px', borderRadius: 12, background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.1)' }}>
                <div style={{ fontSize: 22, fontWeight: 800, color: '#fff', letterSpacing: '-0.5px' }}>{s.v}</div>
                <div style={{ fontSize: 12, color: '#93c5fd', marginTop: 2 }}>{s.l}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '48px 40px', overflowY: 'auto' }}>
        <div className="fade-up" style={{ width: '100%', maxWidth: 420 }}>
          <div style={{ marginBottom: 28 }}>
            <h1 style={{ fontSize: 28, fontWeight: 800, color: '#0f172a', letterSpacing: '-0.8px', marginBottom: 8 }}>Crea tu cuenta</h1>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              {[
                { id: 'analista', label: 'Analista SOC', sub: 'Quiero entrenar', icon: (
                ...
                { id: 'empresa', label: 'Empresa', sub: 'Busco talento', icon: (
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16"/></svg>
                )}
              ].map(r => (
                <div key={r.id} className="rol-card" onClick={() => setForm({ ...form, rol: r.id })}
                  style={{ padding: '14px', borderRadius: 10, background: form.rol === r.id ? '#eff6ff' : '#fff', border: `1.5px solid ${form.rol === r.id ? '#2563eb' : '#e2e8f0'}` }}>
                  <div style={{ color: form.rol === r.id ? '#2563eb' : '#94a3b8', marginBottom: 8 }}>{r.icon}</div>
                  <div style={{ fontSize: 13, fontWeight: 700, color: form.rol === r.id ? '#1e40af' : '#374151' }}>{r.label}</div>
          ...
          <p style={{ textAlign: 'center', fontSize: 13, color: '#64748b', marginTop: 20 }}>
            ¿Ya tienes cuenta?{' '}
            <span className="lnk" onClick={() => navigate('/login')} style={{ color: '#2563eb', cursor: 'pointer', fontWeight: 600 }}>Iniciar sesión</span>
          </p>
        </div>
      </div>
    </div>
