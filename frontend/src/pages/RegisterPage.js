...
const RegisterPage = () => {
  ...
  const css = `
    ...
    .rol-card{transition:border-color .15s,background .15s;cursor:pointer}
  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex', fontFamily:"'Inter',-apple-system,sans-serif" }}>
      ...
      <div style={{ flex:'0 0 52%', background:'linear-gradient(160deg,#0f0c29 0%,#1e1b4b 35%,#24006a 70%,#1e3a8a 100%)', display:'flex', flexDirection:'column', justifyContent:'space-between', padding:'48px 52px', minHeight:'100vh', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ position:'absolute', top:'40%', right:'-40px', width:'180px', height:'180px', borderRadius:'50%', background:'radial-gradient(circle,rgba(192,132,252,0.08) 0%,transparent 70%)', pointerEvents:'none' }}/>
        ...
        <div style={{ position:'relative', zIndex:1, flex:1, display:'flex', flexDirection:'column', justifyContent:'center', padding:'40px 0' }}>
          ...
          <div style={{ display:'inline-flex', alignItems:'center', gap:'8px', padding:'5px 13px', borderRadius:'100px', border:'1px solid rgba(165,180,252,0.25)', backgroundColor:'rgba(79,70,229,0.15)', marginBottom:'28px', width:'fit-content' }}>
            ...
            <span style={{ fontSize:'11px', color:'#a5b4fc', fontWeight:700, letterSpacing:'2px' }}>ÚNETE GRATIS — SIN TARJETA</span>
          </div>

          <h2 style={{ fontSize:'clamp(30px,3.5vw,44px)', fontWeight:900, color:'#fff', lineHeight:1.05, letterSpacing:'-1.5px', marginBottom:'16px' }}>
            Tu carrera SOC<br/>
          ...
          <p style={{ fontSize:'15px', color:'#c7d2fe', lineHeight:1.75, maxWidth:'360px', marginBottom:'36px' }}>
            12 divisiones competitivas. De Bronce III a Diamante I. Certifícate con QR verificable.
          </p>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
            {[
              'Sesiones SOC únicas generadas por IA',
              'XP real que sube tu tier de analista',
              'Ranking global y comunidad activa',
            ].map((item,i)=>(
        ...
        {/* Footer — Powered by Zorion centrado */}
      ...
      {/* ── PANEL DERECHO ── */}
      <div style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center', padding:'40px', background:'linear-gradient(150deg,#f0f4ff 0%,#f8f9ff 50%,#f5f0ff 100%)', overflowY:'auto' }}>
        <div className="fade-up" style={{ width:'100%', maxWidth:'420px' }}>

          <div style={{ marginBottom:'24px' }}>
            <h1 style={{ fontSize:'28px', fontWeight:800, color:'#0f172a', letterSpacing:'-0.8px', marginBottom:'6px' }}>Crea tu cuenta</h1>
            <p style={{ fontSize:'14px', color:'#64748b' }}>Gratis, sin tarjeta de crédito</p>
          </div>
          ...
          <div style={{ marginBottom:'16px' }}>
            <label style={{ fontSize:'13px', fontWeight:600, color:'#374151', display:'block', marginBottom:'8px' }}>Soy un...</label>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px' }}>
              {[
                { id:'analista', label:'Analista SOC', sub:'Quiero entrenar',
                ...
                { id:'empresa',  label:'Empresa',      sub:'Busco talento',
                ...
                <div key={r.id} className="rol-card" onClick={()=>setForm({...form,rol:r.id})}
                  style={{ padding:'14px', borderRadius:'10px', background:form.rol===r.id?'#eef2ff':'#fff', border:`1.5px solid ${form.rol===r.id?'#4f46e5':'#e2e8f0'}`, boxShadow:form.rol===r.id?'0 2px 8px rgba(79,70,229,0.12)':'0 1px 3px rgba(0,0,0,0.04)' }}>
                  <div style={{ color:form.rol===r.id?'#4f46e5':'#94a3b8', marginBottom:'6px' }}>{r.icon}</div>
                  <div style={{ fontSize:'13px', fontWeight:700, color:form.rol===r.id?'#3730a3':'#374151' }}>{r.label}</div>
                  <div style={{ fontSize:'11px', color:'#94a3b8', marginTop:'2px' }}>{r.sub}</div>
                </div>
              ))}
            </div>
          </div>
