...
const LoginPage = ({ onGuestLogin }) => {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #f0f4ff 0%, #fafbff 50%, #f5f0ff 100%)', display: 'flex', fontFamily: "'Inter',-apple-system,sans-serif" }}>
      ...
      {/* LEFT PANEL */}
      <div style={{ flex: 1, background: 'linear-gradient(160deg, #1e1b4b 0%, #312e81 50%, #1e40af 100%)', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '48px', minHeight: '100vh', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative' }}>
          <div style={{ fontSize: 11, color: '#a5b4fc', letterSpacing: '2px', fontWeight: 600, marginBottom: 16 }}>SOC PLATFORM</div>
          ...
          <p style={{ fontSize: 15, color: '#c7d2fe', lineHeight: 1.7, maxWidth: 340, marginBottom: 40 }}>12 divisiones competitivas. Desde Bronce 3 hasta Diamante 1.</p>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            {[{v:'2.400+',l:'Analistas activos'},{v:'12',l:'Divisiones'},{v:'98%',l:'Satisfacción'},{v:'QR',l:'Certificado verificable'}].map((s,i) => (
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '48px 40px' }}>
        <div className="fade-up" style={{ width: '100%', maxWidth: 400 }}>
          <div style={{ marginBottom: 28 }}>
            <h1 style={{ fontSize: 28, fontWeight: 800, color: '#0f172a', letterSpacing: '-0.8px', marginBottom: 6 }}>Bienvenido de nuevo</h1>
            <p style={{ fontSize: 14, color: '#64748b' }}>Inicia sesión en tu cuenta SocBlast</p>
          </div>

          {error && <div style={{ marginBottom: 18, padding: '12px 16px', borderRadius: 10, background: '#fef2f2', border: '1px solid #fecaca', color: '#dc2626', fontSize: 13 }}>{error}</div>}
          ...
          <p style={{ textAlign: 'center', fontSize: 13, color: '#64748b', marginTop: 20 }}>
            ¿No tienes cuenta?{' '}
            <span className="lnk" onClick={() => navigate('/register')} style={{ color: '#4f46e5', cursor: 'pointer', fontWeight: 600 }}>Regístrate gratis</span>
          </p>
        </div>
      </div>
    </div>
